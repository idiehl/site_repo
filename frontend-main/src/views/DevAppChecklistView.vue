...
async function fetchChecklist() {
  ...
  try {
    const response = await fetch(`https://apply.atlasuniversalis.com/api/v1/dev/apps/${appId.value}/checklist`, {
      headers: {
    ...
    error.value = err.message || 'Failed to load checklist'
  } finally {
    loading.value = false
  }
}
...
async function createTask() {
  ...
  if (!taskText) {
    taskError.value = 'Enter a task before saving'
    return
  }
  ...
  try {
    const response = await fetch(`https://apply.atlasuniversalis.com/api/v1/dev/apps/${appId.value}/checklist/tasks`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${accessToken.value}`,
    ...
    taskError.value = err.message || 'Failed to create task'
  } finally {
...
<template>
  <div class="min-h-screen bg-night-950">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div class="flex items-center gap-2">
            <ClipboardDocumentCheckIcon class="w-6 h-6 text-atlas-400" />
            <h1 class="text-xl font-bold text-white">{{ appName }} — Checklist</h1>
          </div>
        </div>
        <div class="flex items-center gap-4">
    ...
    <main class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div v-if="!loading && !error" class="mb-6 bg-night-900 border border-night-800 rounded-xl p-6">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
          <input
            v-model="newTask"
            type="text"
            placeholder="Add a checklist item (saved under Rapid Capture)"
            class="flex-1 px-4 py-2 rounded-lg bg-night-950 border border-night-800 text-white placeholder:text-night-500 focus:outline-none focus:ring-2 focus:ring-atlas-500"
        ...
        <p v-if="taskError" class="mt-3 text-sm text-red-400">{{ taskError }}</p>
      </div>
